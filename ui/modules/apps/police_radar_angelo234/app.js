...
.directive('policeRadarAngelo234', ['$log', 'Utils', function ($log, Utils) {
  return {
    restrict: 'E',
		templateUrl: '/ui/modules/apps/police_radar_angelo234/app.html',
    replace: true,
    link: function (scope, element, attrs) {
      ...
      scope.$on('sendRadarInfo', function (event, data) { 
        ...
        if (!data.radar_xmitting) {
          ...
          document.getElementById('locked-speed-indicator').style.color = "black";
          document.getElementById('fastest-speed-indicator').style.color = "black";
        }
        else{
          //Transmitting
          ...
          switch(data.middle_display_mode) {
            case "fastest_speed":
              ...
              document.getElementById('locked-speed-indicator').style.color = "black";
              document.getElementById('fastest-speed-indicator').style.color = "red";
            ...
            case "locked_speed":
              ...
              document.getElementById('locked-speed-indicator').style.color = "red";
              document.getElementById('fastest-speed-indicator').style.color = "black";
      ...
      scope.$on('streamsUpdate', function (event, streams) {
        ...
        bngApi.activeObjectLua("controller.getController('policeRadar')", (data) => {
          scope.$evalAsync(() => {
						if (document.getElementById('radar-display-content') !== null) {
							if(data !== undefined && data !== null){
								document.getElementById('radar-display-content').style.display = "flex";
							}
							else {
								document.getElementById('radar-display-content').style.display = "none";
							}
						}  
          });  
        });
